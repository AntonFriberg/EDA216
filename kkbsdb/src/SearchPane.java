...
public class SearchPane extends BasicPane {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (card.getName().equals("Date")) {
				...
				if (endDate != null) {
					...
					db.searchByDate(startDateFormatted, endDateFormatted, chosenCookie, chosenIngr, onlyBlocked,palletListModel);
				} else {
					// Här hanterar vi om man bara söker på ett datum.
					db.searchByDate(startDateFormatted, chosenCookie, chosenIngr, onlyBlocked,palletListModel);
				}
			...
			if (card.getName().equals("Number")) {
				...
				db.searchById(input.getText(), chosenCookie, chosenIngr, onlyBlocked,palletListModel);
				input.setText("");
			} else

			if (card.getName().equals("All")) {
				// Söker efter alla pallar
				db.searchByAll(chosenCookie, chosenIngr,onlyBlocked, palletListModel);
	...
	class BlockMarkActionListener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			...
			onlyBlocked=onlyBlocked.equals("")?"AND isBLocked IS NOT NULL":"";
